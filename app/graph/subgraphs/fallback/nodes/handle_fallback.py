...
async def handle_fallback_node(state: GlobalState) -> GlobalState:
    ...
    response_text = await _generate_contextual_response(user_message, conversation_type)
...
async def _generate_contextual_response(user_message: str, conversation_type: str) -> str:
    """
    Generate contextual responses based on conversation type.
    """
    try:
        if conversation_type == "greeting":
            return await _handle_greeting(user_message)
        elif conversation_type == "capabilities":
            return await _handle_capabilities_inquiry(user_message)
        elif conversation_type == "farewell":
            return await _handle_farewell(user_message)
        elif conversation_type == "out_of_scope":
            return await _handle_out_of_scope(user_message)
        elif conversation_type == "smalltalk":
            return await _handle_smalltalk(user_message)
        elif conversation_type == "faq":
            return await _handle_faq(user_message)
        elif conversation_type == "support_query":
            return await _handle_support_query(user_message)
        else:  # unknown
            return await _handle_unknown(user_message)
    except Exception:
        # Fallback to safe default response
        return await _get_default_ecommerce_response(user_message)

async def _handle_greeting(user_message: str) -> str:
    ...
    try:
        ...
        response = await llm.ainvoke(greeting_prompt.invoke({"message": user_message}))
...
async def _handle_capabilities_inquiry(user_message: str) -> str:
    ...
    try:
        ...
        llm = llm_service.get_llm_without_tools()
        response = await llm.ainvoke(capabilities_prompt.invoke({"message": user_message}))
        return str(response.content).strip() if hasattr(response, 'content') else str(response).strip()
    except Exception:
...
async def _handle_farewell(user_message: str) -> str:
    ...
    try:
        ...
        llm = llm_service.get_llm_without_tools()
        response = await llm.ainvoke(farewell_prompt.invoke({"message": user_message}))
        return str(response.content).strip() if hasattr(response, 'content') else str(response).strip()
    except Exception:
        return "Thank you for shopping with us! Feel free to come back anytime for your shopping needs. Have a great day!"

async def _handle_out_of_scope(user_message: str) -> str:
    ...
    try:
        ...
        llm = llm_service.get_llm_without_tools()
        response = await llm.ainvoke(out_of_scope_prompt.invoke({"message": user_message}))
        return str(response.content).strip() if hasattr(response, 'content') else str(response).strip()
    except Exception:
...
async def _get_default_ecommerce_response(user_message: str = "") -> str:
    ...
    try:
        ...
        llm = llm_service.get_llm_without_tools()
        response = await llm.ainvoke(default_prompt.invoke({"message": user_message}))
        return str(response.content).strip() if hasattr(response, 'content') else str(response).strip()
    except Exception:
