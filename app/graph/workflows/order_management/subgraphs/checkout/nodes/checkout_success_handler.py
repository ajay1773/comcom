...
async def checkout_success_handler_node(state: CheckoutState, config: RunnableConfig | None = None) -> CheckoutState:
    ...
    try:
        ...
        success_prompt = ChatPromptTemplate.from_messages([
            ...
            """),
            ("user", f"Order {order_number} placed successfully! Total: ${total_amount:.2f}, {len(cart_items)} items, checkout type: {checkout_type}")
        ])
        ...
        print(f"Checkout success: Order {order_number} - ${total_amount:.2f}")
        
        return state
        
    except Exception as e:
        print(f"Error in checkout success handler: {e}")
